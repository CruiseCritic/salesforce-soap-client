...
use Salesforce\Common\AbstractHasDispatcher;
...
class Client extends AbstractHasDispatcher implements ClientInterface
{
     ...
     * Login result
     *
     * @var \Salesforce\SoapClient\Result\LoginResult
     */
    ...
    public function merge(array $mergeRequests, $type)
    {
        foreach ($mergeRequests as $mergeRequest) {
            if (!($mergeRequest instanceof \Salesforce\SoapClient\Request\MergeRequest)) {
    ...
    public function query($query)
    {
        ...
        return new \Salesforce\SoapClient\Result\RecordIterator($this, $result);
    }
     ...
     *
     * @return array
     * @throws \Salesforce\SoapClient\Exception\SaveException  When Salesforce returned an error
     */
    protected function checkResult(array $results, array $params)
    {
        $exceptions = new \Salesforce\SoapClient\Exception\SaveException();
        ...
        for ($i = 0; $i < count($results); $i++) {
            ...
            if (is_object($params[$i])
                ...
                && $results[$i] instanceof \Salesforce\SoapClient\Result\SaveResult) {
    ...
    protected function call($method, array $params = array())
    {
        ...
        $requestEvent = new RequestEvent($method, $params);
        $this->dispatch($requestEvent, Events::REQUEST);

        try {
            $result = $this->soapClient->$method($params);
        } catch (\SoapFault $soapFault) {
            $faultEvent = new FaultEvent($soapFault, $requestEvent);
        ...
        $this->dispatch(new ResponseEvent($requestEvent, $result->result), Events::RESPONSE);
    ...
    protected function setLoginResult(\Salesforce\SoapClient\Result\LoginResult $loginResult)
    {
